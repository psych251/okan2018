...
bias_summary_d = d %>% 
  ...
  summarise(N=n(),
            Mean_Difference = mean(Bias, na.rm = T),
...
duration_model = lm(Bias ~ Condition_Dummy + Literacy_Score + (Condition_Dummy * Literacy_Score), d)
...
test_summary_d = d %>% 
  group_by(Condition, Literacy_Group) %>%
  summarise(p_value = t.test(Bias)$pvalue)
test_summary_d
```

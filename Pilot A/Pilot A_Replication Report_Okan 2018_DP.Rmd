...
bias_summary_d = d %>% 
  ...
  summarize(MeanBias = mean(Bias, na.rm = T), 
            SDBias = sd(Bias))
bias_summary_d
```
...
#####Replicate Figure 5
```{r include=T}
ggplot(bias_summary_d, aes(x=Condition, y=MeanBias, group=Literacy_Group, color=Literacy_Group)) +
  geom_pointrange(position = position_jitter(width = 0.1, height = 0.1), aes(ymin = MeanBias-SDBias, ymax = MeanBias+SDBias))
